...
void ProgressColor::drawBar(QPainter *painter)
{
    ...
    int textWidth = painter->fontMetrics().horizontalAdvance(strMaxValue);
#else
    int textWidth = painter->fontMetrics().width(strMaxValue);
#endif
    textWidth += 10;
...
//    painter->drawText(textRect, Qt::AlignCenter, strValue);
    ...
    //绘制进度条背景
    QRectF barBgRect(QPointF(space, space), QPointF(width() - space * 2, height() - space));
    painter->setPen(Qt::NoPen);
    painter->setBrush(barBgColor);
